...
- **Portefeuille plateforme** : Commissions (5% des commandes), frais d'annulation (10%) et frais de certification (100%) sont cumulés dans un portefeuille plateforme unique, consultable et retirable par un admin (mêmes principes que le retrait vendeur).
...
- **Initier un Paiement** : `POST /api/paiements/initier`
  - Ouvert à tout utilisateur authentifié, quel que soit son rôle (client, producteur ou admin) — plus restreint au seul rôle client.
  - *Body* :
    ```json
    {
  ...
  - Pour une `CERTIFICATION`, `vendeurId` n'est pas nécessaire (le champ interne est renseigné avec l'id du payeur, à titre d'audit) : la totalité du montant est un revenu direct de la plateforme (pas de séquestre, pas de partage 95/5, contrairement à une `COMMANDE`).

### 🧑‍🌾 Rôle Vendeur (Producteur)
...
- **Vérifier un paiement (Sondage / Polling)** : `GET /api/paiements/{id}/verifier`
  - Interroge l'API NotchPay. Si NotchPay confirme le paiement (`complete`), la transaction passe au statut `PAYE`, et soit le séquestre du vendeur est crédité (`COMMANDE`), soit le portefeuille plateforme l'est directement (`CERTIFICATION`). Renvoie la transaction à jour.
- **Récupérer le statut résumé** : `GET /api/paiements/statut/{typeReference}/{referenceId}`
  - Permet aux services de commandes ou de certifications de vérifier si une entité liée est payée (et, pour une commande, si son séquestre a déjà été libéré) sans exposer de détails sensibles.
...
- **Libérer le séquestre d'une commande livrée** (rôle admin / service interne, appelé par `commande-service`) : `PUT /api/paiements/commandes/{commandeId}/liberer`
- **Rembourser une commande annulée avant expédition** (rôle admin / service interne) : `PUT /api/paiements/commandes/{commandeId}/rembourser-annulation`
  - 90% remboursé au client, 10% retenu par la plateforme (frais d'annulation).
- **Rembourser un litige "Produit non livré"** (rôle admin / service interne) : `PUT /api/paiements/commandes/{commandeId}/rembourser-litige`
  - Remboursement intégral (100%) au client. Refusé (400) si le séquestre a déjà été libéré vers le solde disponible du vendeur.

### 👑 Rôle Administrateur
- **Historique Global** : `GET /api/paiements/admin/toutes-transactions`
  - Liste l'ensemble des transactions, commandes et certifications confondues (utile pour le tableau de bord des ventes et le suivi des commissions de la plateforme).
- **Portefeuille de la plateforme** : `GET /api/paiements/admin/solde-plateforme`
  - Renvoie le cumul historique total gagné (`totalGagne`, commissions commandes + frais d'annulation + frais de certification, ne diminue jamais) et le solde réellement disponible pour un retrait (`soldeDisponible`, qui diminue à chaque retrait plateforme).
- **Retrait plateforme** : `POST /api/paiements/admin/retrait`
  - *Body* : `{ "montant": 10000.00, "methode": "MOMO", "numero": "612345678" }`
  - Débite `soldeDisponible` (jamais `totalGagne`) et génère une référence de virement simulée (`PAYOUT-PLATEFORME-UUID`).
- **Historique des retraits plateforme** : `GET /api/paiements/admin/retraits`
