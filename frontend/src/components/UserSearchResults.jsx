...
export default function UserSearchResults({ onBack, onSelectUser }) {
  ...
  const lancerRecherche = async (e) => {
    ...
    try {
      ...
      setErreur(err?.message || 'La recherche a échoué.');
      setResults([]);
    } finally {
      setChargement(false);
    }
  ...
  return (
    <div style={styles.wrapper}>
      <div style={styles.container}>
        ...
        <h1 style={styles.title}>Rechercher un utilisateur</h1>
        ...
        <form style={styles.searchWrap} onSubmit={lancerRecherche}>
          ...
          <input
            ...
            placeholder="Nom d'un client ou d'un producteur..."
            value={query}
        ...
        {chargement && <p style={styles.hint}>Recherche en cours...</p>}

        {!chargement && results !== null && (
          results.length === 0 ? (
            <p style={styles.emptyText}>Aucun utilisateur trouvé pour "{query}"</p>
          ) : (
                ...
                <button key={u.id} style={styles.resultCard} onClick={() => onSelectUser(u)}>
                  ...
                  <div style={styles.resultInfo}>
                    ...
                    <span style={styles.roleTag}>
                      {u.role === 'vendeur' ? '🌾 Producteur' : '🛒 Client'}
                    </span>
                  </div>
                </button>
              ))}
